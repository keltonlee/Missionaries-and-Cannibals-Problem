...
class State {
  ...
  public:
    State (int numCann, int numMiss, int boat1, int boat2); //��䬰1 
    int step;
...
vector<State*> nextState(State *start){
    ...
    int boat2 = state[3];
    vector<State*> next;
    for (int Am=0;Am<=2;Am++){     //��A
        for (int Ac=0;Am + Ac<=2;Ac++){
            for (int Bm=0;Bm<=3;Bm++){   //��B
...
bool notFound(State* start, list<State*> & OPEN, list<State*> & CLOSE){
    ...
    for(; itr1 != OPEN.end() ; itr1++){
        if( ((*itr1)->getStateAttr()[0]==start->getStateAttr()[0]) &&  ((*itr1)->getStateAttr()[1]==start->getStateAttr()[1]) && ((*itr1)->getStateAttr()[2]==start->getStateAttr()[2]) && ((*itr1)->getStateAttr()[3]==start->getStateAttr()[3]))  break;
    }
    for (; itr2 != CLOSE.end(); itr2++)
    {
        if( ((*itr2)->getStateAttr()[0]==start->getStateAttr()[0]) &&  ((*itr2)->getStateAttr()[1]==start->getStateAttr()[1]) && ((*itr2)->getStateAttr()[2]==start->getStateAttr()[2]) && ((*itr2)->getStateAttr()[3]==start->getStateAttr()[3]))  break;
    }

    if( (itr1 == OPEN.end()) && (itr2 == CLOSE.end()) )
        return true; 
    // if((itr1 != OPEN.end()) && ((*itr1)->getgn() + (*itr1)->gethn() > start->getgn() + start->gethn())){
    //     return true;
    // }
    // if((itr2 != CLOSE.end()) && ((*itr2)->getgn() + (*itr2)->gethn() > start->getgn() + start->gethn())){
    //     return true;
    // }                       
    return false;
}

bool notFound2(State* start, list<State*> & OPEN, list<State*> & CLOSE){
    list<State*>::iterator itr1 = OPEN.begin();
    list<State*>::iterator itr2 = CLOSE.begin();
    for(; itr1 != OPEN.end() ; itr1++){
        if( ((*itr1)->getStateAttr()[0]==start->getStateAttr()[0]) &&  ((*itr1)->getStateAttr()[1]==start->getStateAttr()[1]) && ((*itr1)->getStateAttr()[2]==start->getStateAttr()[2]) && ((*itr1)->getStateAttr()[3]==start->getStateAttr()[3]))  break;
...
State* dijkstra(list<State*> & OPEN, list<State*> & CLOSE){     //�D����̧C
...
State* astar(list<State*> & OPEN, list<State*> & CLOSE){     //�D�̵u���|
    while(!OPEN.empty()){
        OPEN.sort(astarop);
        State *root;
        root = OPEN.front();
...
State* bfs(list<State*> & OPEN, list<State*> & CLOSE){     //�D�̵u���|
    while(!OPEN.empty()){
        State *root;
        root = OPEN.front();
        ofs<<"----------------------------------"<<endl;
...
State* bfs_limit_cost(list<State*> & OPEN, list<State*> & CLOSE){ 
    while(!OPEN.empty()){
        State *root;
        root = OPEN.front();
        ofs<<"----------------------------------"<<endl;
        ...
        for(int i=0; i<vec.size();i++){
            if(vec[i]->isLegalState() && notFound2(vec[i], OPEN, CLOSE) && vec[i]->getStateAttr() != ini && vec[i]->getgn() <= limit_cost){
                OPEN.push_back(vec[i]);
            }
        }
        if(OPEN.empty()){
...
State* dijkstra_limit_time(list<State*> & OPEN, list<State*> & CLOSE){    
    while(!OPEN.empty()){
        OPEN.sort(op);
        State *root;
        root = OPEN.front();
        ...
        ini.push_back(1);
        for(int i=0; i<vec.size();i++){
            if(vec[i]->isLegalState()  && notFound(vec[i], OPEN, CLOSE) && vec[i]->getStateAttr() != ini && vec[i]->step <= limit_time){
                OPEN.push_back(vec[i]);
            }
        }
    }
...
int main(){
    ...
    cout << "fastest type 1, least cost type 2, astar type 3:";
    cin >> bfs_or_dij;

    cout << "cost limit type 1, time limit type 2, no limits type 3:";
    int limit;
    cin >> limit;
    State *final;
    if(limit == 1){
        ...
        final = bfs_limit_cost(OPEN, CLOSE);
        diff = (double)(clock()-tstart)/CLOCKS_PER_SEC;
    }
    else if(limit == 2){
        cout << "limit time: ";
        cin >> limit_time; 
        clock_t tstart = clock();
        final = dijkstra_limit_time(OPEN, CLOSE);
        diff = (double)(clock()-tstart)/CLOCKS_PER_SEC;
    }
    else{
        if(bfs_or_dij==1){
    ...
    if(final == nullptr){
       ...
       for(int i = final_list.size() - 1;i>=0;i--){
            cout << "[";
            for (int j = 0; j < final_list[i][0]; j++)
            {
                cout << "X";
            }
            for (int j = 0; j < final_list[i][1]; j++)
            {
                cout << "O";
            }
            cout << "]";
            ...
            if (final_list[i][3]==-1)
                cout << "B";
            cout << "[";
            for (int j = 0; j < totalNumCann - final_list[i][0]; j++)
            {
                cout << "X";
            }
            for (int j = 0; j < totalNumMiss-   final_list[i][1]; j++)
            {   
                cout << "O";    
            }   
            cout << "]" << endl;    
